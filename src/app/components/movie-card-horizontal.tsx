...
export function MovieCardHorizontal({movie}: Props) {
    return <Link href={`/movie/${movie.id}`}>
        <div
            className="flex w-[150px] flex-col shadow-md rounded-md overflow-hidden hover:scale-102 duration-300 h-full">
            <Image width={500} height={600} alt={`Poster of ${movie.title}`}
